...
def funcion_principal():
    " Función principal que inicia el bucle principal del programa. "
    try:
        # Configurar logging
        logger = get_logger()
        ...
        if "--verbose" in sys.argv:
            ...
            logger.debug("Modo debug verbose activado por argumento de línea de comandos")

        # Registrar información del sistema
        logger.debug(f"Sistema operativo: {platform.system()} {platform.release()}")
        logger.debug(f"Versión Python: {platform.python_version()}")
